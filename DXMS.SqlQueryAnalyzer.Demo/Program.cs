...
using DXMS.SqlQueryAnalyzer.Enums;
...
// Test with json
Console.WriteLine("\n" + new string('=', 80));
Console.WriteLine("TEST WITH JSON:");
Console.WriteLine(new string('=', 80));
...
var jsonResult = analyzer.Analyze(jsonQuery);
...
static void PrintResult(QueryAnalysisResult result)
{
    ...
    foreach (var table in result.Tables)
    {
        ...
        var directLabel = table.DirectReference ? "" : " [SUBQUERY]";
        Console.WriteLine($"   [{typeLabel}] {table.FullName}{(table.Alias is not null ? $" AS {table.Alias}" : "")}{joinLabel}{directLabel}");
    }
